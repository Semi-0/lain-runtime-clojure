(ns propagators.runtime.session.input
  "Runtime input commit and replay logic."
  (:require [propagators.runtime.session.block-model :as block-model]
            [propagators.runtime.boundary.effects :as effects]
            [propagators.runtime.inspection.graph-projection :as graphp]
            [propagators.runtime.session.program :as program]
            [propagators.runtime.session.program.rebuild :as program-rebuild]
...
(defn extend-source!
  ...
  (locking session
    (let [forms (read-source-forms source)
          compile-form
          (requiring-resolve
           'propagators.runtime.session.environment-io/compile-hidden-form)]
      (loop [current @session
             remaining forms
             installed []]
        (if-let [form (first remaining)]
          (let [{next-state :state failure :error}
                (compile-form current external-source-client-id form
                              {:client-id client-id :kind :external-source})]
            (if failure
              (do
                ...
                {:client-id client-id
                 :blocks installed
                 :error failure})
              (let [settled (perform-boundary-effects next-state)
                    block (peek (:external-sources settled))]
                (recur settled (next remaining) (conj installed block)))))
          (do
            (reset! session current)
            {:client-id client-id
             :blocks installed}))))))
