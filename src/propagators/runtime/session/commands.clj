(ns propagators.runtime.session.commands
...
(defn- dispatch-extension-command
  [session command]
  (let [op (:op command)
        handler (get-in @session [:runtime/options :command-handlers op])]
    (if (fn? handler)
    ...
    (let [response (locking session
                     {:ok true
                      ...
                      (case op
                        ...
                        :tui/focus-block (focus-block! session command)
                        ...
                        (dispatch-extension-command session command))})]
