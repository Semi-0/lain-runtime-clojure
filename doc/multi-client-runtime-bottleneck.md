...
```text
server/request
-> propagators.runtime.session.commands/handle-command!
...
2. `program/compile-program-form` computes semantic graph data for the new
   ...
   not an immediately-expanded semantic propagators.tui.graph.
...
not eagerly recompute the whole propagators.tui.graph.
...
```bash
clojure -M:test propagators.tui.graph.vijual.compiler-2-runtime-server-test
clojure -M:test propagators.tui.graph.xr-runtime-test
