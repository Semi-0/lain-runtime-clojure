...
- send widget events to compiler-declared widget IO propagators.infra.
...
`propagators.tui.graph.xr-server` serves a small browser UI and a WebSocket endpoint. The
server delegates runtime operations to `propagators.tui.graph.xr-runtime`, which wraps the
...
```text
clojure -M -m propagators.tui.graph.xr-server
```
...
```text
clojure -M -m propagators.tui.graph.xr-server --lan
```
...
```text
clojure -M -m propagators.tui.graph.xr-server --lan --https --keystore xr-local.p12 --keystore-password changeit
```
